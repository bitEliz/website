...
export function useBreakpoints(record?: Record<BreakpointLabel, number>) {
  ...
  return {
    lessThan: (label: BreakpointLabel) => useMediaQuery(`(max-width: ${getValue(label, -0.02)})`),
    lessThanOrEqual: (label: BreakpointLabel) => useMediaQuery(`(max-width: ${getValue(label)})`),
    greaterThan: (label: BreakpointLabel) => useMediaQuery(`(min-width: ${getValue(label, 0.02)})`),
    ...
    between: (lower: BreakpointLabel, upper: BreakpointLabel) =>
      useMediaQuery(`(min-width: ${getValue(lower)}) and (max-width: ${getValue(upper)})`)
  }
}
